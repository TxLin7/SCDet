...
def compute_loss(p, targets, model, con_info=None):  # predictions, targets, model
    device = p[0].device
    # detection loss
    ...
    # contrastive loss
    lcon = torch.zeros(1, device=device)  # Tensor(0)
    #
    ...
    if con_info is None:
        ...
        if use_coord:
            N = p[0].shape[0]
            coord1 = torch.empty([N, 4])
            coord2 = torch.empty([N, 4])
            for j in range(N):
                coord1[j, :] = torch.tensor(get_coord(model.img_size))
                coord2[j, :] = torch.tensor(get_coord(model.img_size))
            coord1 = coord1.to(device)
            coord2 = coord2.to(device)
        else:
            coord1, coord2 = None, None
            
        if model.pixpro_ins_loss_weight > 0.:
            n = 0
            for i in con_info:
                proj_1, proj_2, pred_1, pred_2 = i
                
                if n == len(con_info)-1:
                    if n != 0:
                        lcon /= len(con_info) - 1
                    loss_instance = (regression_ins_loss(pred_1, proj_2) + regression_ins_loss(pred_2, proj_1))/2
                    lcon += model.pixpro_ins_loss_weight * loss_instance
                else:
                    # lcon += 0.5 * (regression_loss(pred_1, proj_2, model.pixpro_pos_ratio, coord_q=coord1, coord_k=coord2, use_coord=True, image_size=model.img_size)
                    #         + regression_loss(pred_2, proj_1, model.pixpro_pos_ratio, coord_q=coord2, coord_k=coord1, use_coord=True, image_size=model.img_size))
                    # lcon += 0.5 * (regression_loss(pred_1, proj_2, model.pixpro_pos_ratio, coord_q=coord1, coord_k=coord1, use_coord=use_coord,
                    #                                image_size=model.img_size)
                    #                + regression_loss(pred_2, proj_1, model.pixpro_pos_ratio, coord_q=coord1, coord_k=coord1, use_coord=use_coord,
                    #                                  image_size=model.img_size))
                    lcon += 0.5 * (regression_target_loss(pred_1, proj_2, model.pixpro_pos_ratio, target=targets,
                                                          image_size=model.img_size)
                                   + regression_target_loss(pred_2, proj_1, model.pixpro_pos_ratio, target=targets,
                                                            image_size=model.img_size))
                    n += 1
            ...
            for i in con_info:
                ...
                # lcon += 0.5 * (regression_loss(pred_1, proj_2, model.pixpro_pos_ratio, coord_q=coord1, coord_k=coord2, use_coord=use_coord,
                #                                image_size=model.img_size)
                #                + regression_loss(pred_2, proj_1, model.pixpro_pos_ratio, coord_q=coord2, coord_k=coord1, use_coord=use_coord,
                #                                  image_size=model.img_size))
                lcon += 0.5 * (regression_loss(pred_1, proj_2, model.pixpro_pos_ratio, coord_q=coord1, coord_k=coord1, use_coord=use_coord,
                                               image_size=model.img_size)
                               + regression_loss(pred_2, proj_1, model.pixpro_pos_ratio, coord_q=coord1, coord_k=coord1, use_coord=use_coord,
                                                 image_size=model.img_size))
                # lcon += 0.5 * (regression_target_loss(pred_1, proj_2, model.pixpro_pos_ratio, target=targets, image_size=model.img_size)
                #                + regression_target_loss(pred_2, proj_1, model.pixpro_pos_ratio, target=targets, image_size=model.img_size))
            lcon /= len(con_info)
     
        lcon *= 5
        
        # loss = lbox + lobj + lcls + lcon
        return {"box_loss": lbox,
                "obj_loss": lobj,
                "class_loss": lcls,
                "contrastive": lcon}
...
def build_targets(p, targets, model):
    ...
    YOLO_layers = model.YOLO_layers if model.name == "Baseline" else model.yolo.YOLO_layers
...
def regression_ins_loss(x, y):
    return 1. - torch.einsum('nc, nc->n', [x, y]).mean()

def regression_loss(q, k, pos_ratio=0.5, coord_q=None, coord_k=None, use_coord=False, image_size=[416, 416]):
    """ q, k: N * C * H * W
    ...
    if isinstance(image_size, int):
        ...
        img_size = image_size
    if (coord_q==None and coord_k==None) or use_coord == False:
        coord = torch.empty(N, 4)
        for i in range(N):
            coord[i] = torch.tensor([0., 0., img_size[0], img_size[1]])
            coord_q = coord.to(q.device)
            coord_k = coord.to(q.device)
    else:
        coord_q = coord_q.to(q.device)
        coord_k = coord_k.to(q.device)
...
def regression_target_loss(q, k, pos_ratio=0.5, target=None, image_size=[416, 416]):
    """ q, k: N * C * H * W
        coord_q, coord_k: N * 4 (x_upper_left, y_upper_left, x_lower_right, y_lower_right)
    """
    N, C, H, W = q.shape
    ...
    coords = torch.empty([target.shape[0], 4]).to(q.device)

    coords[:, 0] = img_size[0] * (target[:, 2] - target[:, 4] / 2)
    coords[:, 1] = img_size[1] * (target[:, 3] - target[:, 5] / 2)
    coords[:, 2] = img_size[0] * (target[:, 2] + target[:, 4] / 2)
    coords[:, 3] = img_size[1] * (target[:, 3] + target[:, 5] / 2)
    # generate center_coord, width, height
    coord_q, coord_k = coords.to(q.device), coords.to(q.device)
    # [1, 7, 7]
    x_array = torch.arange(0., float(W), dtype=coord_q.dtype, device=coord_q.device).view(1, 1, -1).repeat(1, H, 1)
    y_array = torch.arange(0., float(H), dtype=coord_q.dtype, device=coord_q.device).view(1, -1, 1).repeat(1, 1, W)
    # [bs, 1, 1]
    ...
    img_index = target[:, 0]
    new_logit = torch.empty(img_index.shape[0], logit.shape[1], logit.shape[2]).to(q.device)
    
    for i in range(len(img_index)):
        new_logit[i] = logit[int(img_index[i])]
        
    loss = (new_logit * pos_mask).sum(-1).sum(-1) / (pos_mask.sum(-1).sum(-1) + 1e-6)

    return 1.0 - loss.mean()

def get_coord(img_size, scale=(0.2, 1.0), ratio=(3. / 4., 4. / 3.)):
